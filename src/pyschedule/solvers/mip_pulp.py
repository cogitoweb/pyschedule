...
"""
a simple interface to pulp which is also an interface
blueprint for potential other mip-interfaces
"""
...
class MIP(object):
    ...
    def con(self, affine, sense=0, rhs=0):
        ...
        con = pl.LpConstraint(pl.LpAffineExpression(
            affine), sense=sense, rhs=rhs)
        self.mip += con
        return con
    ...
    def solve(self, msg=0, **kwarg):
        ...
        if kind == 'CPLEX':
            ...
            if time_limit is not None:
                if ratio_gap is not None:
                    self.mip.solve(
                        pl.GUROBI(msg=msg, timeLimit=time_limit, epgap=ratio_gap))
